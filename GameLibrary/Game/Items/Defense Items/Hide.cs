...
namespace Game.Items.Defense_Items
{
    class Hide : IDefenseItem
    {
        public Hide()
        {
            Name = "Hide";
            Slot = "Skin";
            Defense = 1;
            Types = new []{new PhysicalDamageType()};
        }
        public Hide(string name, string slot, double defense, IEnumerable<IDamageType> types)
        {
            Name = name;
            Slot = slot;
            Defense = defense;
